...
A "match" constraint is used to mark where the lyric phrases are intentionally repeated between verses or choruses (even with very slight variations). For exactBinaryMatch, the first LYRIC denotes the first lyric in a sequence of lyrics that are intentional identical sequences of lyrics and a second LYRIC line denotes the first lyric not included in the identical sequence. Even repetitive lyrics across choruses should be marked with this constraint, once for each time the chorus is sung. For example,

match
You	48	1 // start position
friend	34	3 // end position (excluded)
