...
//	List<Map<Integer, Integer>> outSupport; // first matrix represents the number of non-zero transition probabilities from the ith state at pos 0 in the seq
...
//		this.logPriors = new HashMap<Integer, Double>(model.logPriors.size());
		this.inSupport = new ArrayList<Map<Integer, Integer>>(length-1);
//		this.outSupport = new ArrayList<Map<Integer, Integer>>(length-1);
//
		for (int i = 0; i < length-1; i++) {
			this.inSupport.add(new HashMap<Integer, Integer>());
//			this.outSupport.add(new HashMap<Integer, Integer>());
		}
		
		logTransitions = new ArrayList<Map<Integer, Map<Integer, Double>>>(length-1);
		for (int i = 0; i < length-1; i++) {
