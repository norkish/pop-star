...
//		this.key = normalizeChords(this.key);
	...
	public Pair<List<SortedMap<Integer, Chord>>, List<String>> validate(String consensus) {
		...
		System.out.println(consensus.replaceAll("\n", "\\\\n"));
		System.out.println(tabLyrics.replaceAll("\n", "\\\\n"));
		...
		System.out.println("\nAlignment:");
		System.out.println(alnConsensus.replaceAll("\n", "#"));
		System.out.println(alnTabLyrics.replaceAll("\n", "#"));
		...
		if ( lyricCharMatchCount / (double) consensus.length() < VALID_LYRIC_SHEET_THRESH) {
			System.err.println("" + lyricCharMatchCount + " matching lyric chars, less than " + VALID_LYRIC_SHEET_THRESH + " * " +consensus.length() + ", the length of the lyric sheet");
			return null;
		}
